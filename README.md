...
Use the helper in [`site/clear-stylus-console.js`](site/clear-stylus-console.js)
to delete only styles generated by this repo. It previews by default and only
deletes styles with the exact generated marker unless you explicitly enable the
legacy mode for old unmarked imports.

1. Open Stylus → **Manage**.
2. Open the browser DevTools console on that Stylus page.
3. Paste the contents of `site/clear-stylus-console.js`.
4. Review the preview.
5. Run `clearGruvboxStyles({ delete: true })` to delete marker-matched styles.

The helper downloads a full Stylus backup first and requires typing an exact
confirmation phrase before deleting anything. Old unmarked imports are only
listed; to include them, run
`clearGruvboxStyles({ delete: true, legacy: true })` after checking the preview.
